    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = () => {
        ...
        if (!db.objectStoreNames.contains('collection_items')) {
          const collectionItemsStore = db.createObjectStore('collection_items', { keyPath: ['collection_id', 'funko_id'] });
          collectionItemsStore.createIndex('collection_id', 'collection_id', { unique: false });
          collectionItemsStore.createIndex('funko_id', 'funko_id', { unique: false });
        }
      };
    });
  }
  ...
  async getAllFunkos(): Promise<Funko[]> {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        funkos.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
        resolve(funkos);
      };
